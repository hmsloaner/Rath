...
def cal_StaticMeasure(dimensions:VectorStr,measures:VectorStr,dataSource:object,langType:str)->Tuple[float,dict]:
    # static parameters
    static_dict = {}
    score = 0
    para = {}
...
def cal_StaticDimension(dimensions:VectorStr,measures:VectorStr,dataSource:object,langType:str)->Tuple[float,dict]:
    ...
    for D in dimensions:
        ...
        print(countData)
        if len(countData)<=3:
            LowD[D] = {'countData':countData,'maxcount':countData.max()}
        else:
            HighD[D] = {'countData':countData,'maxcount':countData.max()}
    if len(LowD)>=2:
        countMaxList = [-LowD[i]['maxcount'] for i in list(LowD)]
        index = np.argsort(countMaxList)
        keyWether = {}
        for i in index:
            D = list(LowD)[i]
            countData = LowD[D]['countData']
            keyE = countData.index[0]
            keyNum = countData.values[0]
            keyPercent = countData.values[0]/countData.values.sum()
            keyWether[keyE] = dataSource[D]==keyE
            keyLow[D] = {'dimension':D,'key':keyE,'keyNum':keyNum,'keyPercent':keyPercent}
        keyWether = pd.DataFrame(keyWether)
        for i in index:
            D = list(LowD)[i]
            thisK = keyLow[D]['key']
            otherK = [keyLow[j]['key'] for j in list(keyLow)]
            otherK.remove(thisK)
...
def cal_SimpsonParadoxV3(dimensions:VectorStr,breakdown:VectorStr,measures:VectorStr,\
    ...
    if np.std(whitened[:,0])==0 or np.std(whitened[:,1])==0:
        ...
        if score>0:
            explain = explain_SimpsonParadoxV3(langType,measures[1],measures[0],keyDimension)
            # explain = measures[1]+' for '+measures[0]+' shows simpson paradox'
            para['explain'] = explain
    return score,para
...
# explain for en/cn
def explain_StaticDimension(langType,keyLow,keyHigh):
    if langType=='en':
        explain = ''
    if langType=='cn':
        explain = ''
        if len(keyLow)>0:
            for D in list(keyLow):
                if len(explain)>0:
                    explain = explain+'，'
                explain = explain+keyLow[D]['key']+'在'+D+'中有'+str(keyLow[D]['keyNum'])+'个，占'+str("%.2f" % (keyLow[D]['keyPercent']*100))+'%，仅为'+keyLow[D]['key']\
                +'的占比为'+str("%.2f" % (keyLow[D]['keyDesPercent']*100))+'%'
        if len(keyHigh)>0:
            for D in list(keyHigh):
                if len(explain)>0:
                    explain = explain+'，'
                explain = explain+D+'中的TOP3为'+'、'.join([str(j) for j in keyHigh[D]['top3_name']])+'，占比分别为'+'、'.join([str("%.2f" % (j*100))\
                                                                                                   +'%' for j in keyHigh[D]['top3_percent']])
    return explain

def explain_StaticMeasure(langType,static_dict):
    if langType=='en':
        explain = ''
        for M in static_dict:
            if len(explain)>0:
                explain = explain+', '
            explain = explain+'The average value of '+M+' is '+str("%.2f" % static_dict[M]['mean'])+', median value is'+str("%.2f" % static_dict[M]['median'])
    if langType=='cn':
        explain = ''
        for M in static_dict:
            if len(explain)>0:
                explain = explain+'，'
            explain = explain+M+'的平均值是'+str("%.2f" % static_dict[M]['mean'])+'，中位数是'+str("%.2f" % static_dict[M]['median'])
    return explain

def explain_2DClustering(langType,measure0,measure1,aggr_mark,aggrType,breakdown):
    if langType=='en':
        explain = 'There is some clustering between '+measure0+' and '+measure1
        if aggr_mark:
...
def explain_CrossMeasureCorrelation(langType,measure0,measure1,aggr_mark,aggrType,breakdown):
    if langType=='en':
        explain = measure0+' will grow as '+measure1+' grows'
        if aggr_mark:
            explain = explain+' after '+aggrType+' with '+breakdown[0]
    if langType=='cn':
        explain = ''
...
def explain_Correlation(langType,subspaces_name0,subspaces_name1,aggrType,breakdown):
    if langType=='en':
        explain = subspaces_name0+' will grow as '+subspaces_name1+' grows after '+aggrType+' with '+breakdown
    if langType=='cn':
        explain = '以'+breakdown+'求'+aggrType+'的聚合后，'+subspaces_name0+'会随着'+subspaces_name1+'的变化而变化'
    return explain

def explain_Attribution(langType,measure0,aggrType,groupDataindex,breakdown):
    if langType=='en':
        explain = groupDataindex+' has noticeably more '+measure0+' after '+aggrType+' with '+breakdown
    if langType=='cn':
        explain = groupDataindex+'对应的'+measure0+'的'+aggrType+'，在'+breakdown+'中占比很大'
    return explain
...
def explain_OutstandingNo2(langType,measure0,aggrType,groupDataindex0,groupDataindex1,breakdown):
    if langType=='en':
        explain = groupDataindex0+' and '+groupDataindex1+' have noticeably more '+measure0+' after '+aggrType+' with '+breakdown
    if langType=='cn':
        explain = groupDataindex0+'和'+groupDataindex1+'对应的'+measure0+'的'+aggrType+'，在'+breakdown+'中占比很大'
    return explain
...
def explain_HeteroscedasticityV1(langType,measure0,aggrType,breakdown):
    if langType=='en':
        explain = 'The value fluctuation of '+measure0+' will gradually increase with '+breakdown
    if langType=='cn':
        explain = measure0+'的数值波动会随着'+breakdown+'逐渐增加'
    return explain

def explain_HeteroscedasticityV2(langType,measure0,aggrType,breakdown):
    if langType=='en':
        explain = 'The value fluctuation of '+measure0+' will gradually decrease with '+breakdown
    if langType=='cn':
        explain = measure0+'的数值波动会随着'+breakdown+'逐渐减小'
    return explain

def explain_SimpsonParadoxV1(langType,measure0,measure1):
    if langType=='en':
        explain = 'The overall trend of '+measure0+' under '+measure1+' is different from subset'
    if langType=='cn':
        explain = measure1+'在'+measure0+'下的整体趋势与其中子类别的趋势存在异常'
    return explain

def explain_SimpsonParadoxV2(langType,measure0,measure1,aggrType,breakdown):
    if langType=='en':
        explain = 'The overall trend of '+measure0+' under '+measure1+' is different from subset after '+aggrType+' with '+breakdown
    if langType=='cn':
        explain = '分别以'+breakdown+'进行'+aggrType+'聚合后,'+measure1+'在'+measure0+'下的整体趋势与其中子类别的趋势存在异常'
    return explain

def explain_SimpsonParadoxV3(langType,measure0,measure1,keyDimension):
    if langType=='en':
        explain = measure0+' for '+measure1+' shows different distribution with and without '+keyDimension
    if langType=='cn':
        explain = keyDimension+'在'+measure1+'和'+measure0+'下的分布和整体分布存在差异'
    return explain
...
def explain_NolinearrelationshipV1(langType,measure0,measure1,aggrType,breakdown):
    ...
    if langType=='cn':
        explain = '在以'+breakdown+'求'+aggrType+'聚合后,'+measure1+'与'+measure0+'之间存在较为明显的函数关系'
    return explain
...
def insight_check(fields:object,dataSource:object,check_list:VectorStr=None,breakdown:VectorStr=None,\
    ...
    score,para = cal_StaticMeasure(Oridimensions,measures,dataSource,langType)
    insight_dict['StaticMeasure'] = {'score':score,'para':para}
    score,para = cal_StaticDimension(Oridimensions,measures,dataSource,langType)
    insight_dict['StaticDimension'] = {'score':score,'para':para}
    tmp = '2DClustering','CrossMeasureCorrelation','Correlation','Attribution','OutstandingNo1','OutstandingNo2','OutstandingLast','Evenness','ChangePoint','Outlier','Seasonality','Trend','HeteroscedasticityV1','HeteroscedasticityV2'
    
    if True: # if '2DClustering' in check_list:
        if len(measures)>=2:
    ...
    if True:  # if 'CrossMeasureCorrelation' in check_list:
        if len(measures)>=2:
            subDataSource = dataSource
    ...
    if True: # if 'Correlation' in check_list:
    ...
    if True: # if 'Attribution' in check_list:
    ...
    if True: # if 'OutstandingNo1' in check_list:
        if len(measures)>=1 and breakdown is not None:
            subDataSource = dataSource
    ...
    if True: # if 'OutstandingNo2' in check_list:
        if len(measures)>=1 and breakdown is not None:
            subDataSource = dataSource
    ...
    if True: # if 'OutstandingLast' in check_list:
        if len(measures)>=1 and breakdown is not None:
            subDataSource = dataSource
    ...
    if True: # if 'Outlier' in check_list:
        if len(measures)>=1 and breakdown is not None:
            subDataSource = dataSource
            score,para = cal_Outlier(breakdown,measures,subDataSource,aggrType,langType)
            insight_dict['Outlier'] = {'score':score,'para':para}
    if True: # if 'Seasonality' in check_list and breakdown is not None:
        if breakdown is not None and len(measures)>=1 and pd.to_datetime(dataSource.groupby(breakdown).count().index,\
    ...
    if True: # if 'HeteroscedasticityV2' in check_list:
        if len(measures)>=1 and breakdown is not None:
            subDataSource = dataSource
    ...
    if True: # if 'SimpsonParadoxV1' in check_list:
        try:
            if len(measures)>=2:
                subDataSource = dataSource
            ...
            pass
    if True: # if 'SimpsonParadoxV2' in check_list:
    ...
    if True: # if 'SimpsonParadoxV3' in check_list:
    ...
    if True: # if 'NolinearrelationshipV1' in check_list:
        if len(measures)>=2 and breakdown is not None:
            subDataSource = dataSource
    ...
    if True: # if 'scagnostics' in check_list:
        if len(measures)>=2:
            subDataSource = dataSource
